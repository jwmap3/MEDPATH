...
function dispatch(){
  ...
  if(state.step === 'drugQuiz') return renderDrugQuiz();
...
function renderHome(){
  ...
  document.getElementById('drugQuizBtn').addEventListener('click', () => {
    state.step = 'drugQuiz'; render();
  });
...
// ---------- Drug Identity Test ----------
// Every question and every answer choice below is generated at runtime,
// straight from state.drugs (the same drugs.json that drives the drug
// reference screen). Nothing here is hand-authored text, so the quiz can
// never drift out of sync with — or contradict — the source protocol data.
const DRUG_QUIZ_TOTAL = 150;
const quizState = {
  active: false,
  ended: false,
  questions: [],
...
function pickDistractors(pool, exclude, n){
  ...
  return shuffleArr(filtered).slice(0, n);
}

// Build the full candidate pool of faithful questions from the loaded
// drug data. Every "answer" and every "option" is a value copied directly
// out of a drug record — never invented.
function buildDrugQuizPool(){
  const drugs = state.drugs || [];
  const pool = [];
  if(!drugs.length) return pool;

  const allMechanisms = [];
  drugs.forEach(d => { if(d.mechanism_of_action && allMechanisms.indexOf(d.mechanism_of_action) === -1) allMechanisms.push(d.mechanism_of_action); });
  const allNames = drugs.map(d => d.name);

  const mechCount = {};
  drugs.forEach(d => {
    if(!d.mechanism_of_action) return;
    mechCount[d.mechanism_of_action] = (mechCount[d.mechanism_of_action] || 0) + 1;
  });

  // A — "Presents a drug, asks what type of drug it is"
  drugs.forEach(d => {
    if(!d.mechanism_of_action) return;
    const distractors = pickDistractors(allMechanisms, d.mechanism_of_action, 3);
    if(distractors.length < 3) return;
    pool.push({
      category: 'Classification',
      prompt: `What type of drug is ${d.name}?`,
      answer: d.mechanism_of_action,
      options: shuffleArr([d.mechanism_of_action, ...distractors]),
    });
  });

  // B — "Presents a classification, asks how it works" — shown as the
  // drug's own mechanism/classification text, asking which medication it
  // describes. Only generated when that exact mechanism text belongs to a
  // single drug, so there is always exactly one correct answer.
  drugs.forEach(d => {
    if(!d.mechanism_of_action) return;
    if(mechCount[d.mechanism_of_action] !== 1) return;
    const distractors = pickDistractors(allNames, d.name, 3);
    if(distractors.length < 3) return;
    pool.push({
      category: 'Mechanism',
      prompt: `Which medication works this way: "${d.mechanism_of_action}"?`,
      answer: d.name,
      options: shuffleArr([d.name, ...distractors]),
    });
  });

  // C — "Presents a situation, asks which drug would be used" — built from
  // each drug's own listed indications. Only generated when that exact
  // indication text is unique to a single drug in the formulary, so the
  // scenario always has one unambiguous right answer.
  const indicationDrugs = {};
  drugs.forEach(d => {
    const blocks = [...(d.adult_dosing || []), ...(d.pediatric_dosing || [])];
    blocks.forEach(b => {
      if(!b.indication) return;
      b.indication.split(' / ').forEach(part => {
        const key = part.trim();
        if(!key) return;
        if(!indicationDrugs[key]) indicationDrugs[key] = [];
        if(indicationDrugs[key].indexOf(d.name) === -1) indicationDrugs[key].push(d.name);
      });
    });
  });
  Object.keys(indicationDrugs).forEach(ind => {
    const names = indicationDrugs[ind];
    if(names.length !== 1) return;
    const drugName = names[0];
    const distractors = pickDistractors(allNames, drugName, 3);
    if(distractors.length < 3) return;
    pool.push({
      category: 'Scenario',
      prompt: `A patient presents with: ${ind}. Which medication is indicated per protocol?`,
      answer: drugName,
      options: shuffleArr([drugName, ...distractors]),
    });
  });

  // D — straightforward recall: the exact adult dosing text for a given
  // indication (dose amount, concentration, and route all as written).
  const allDoseTexts = [];
  drugs.forEach(d => {
    (d.adult_dosing || []).forEach(b => {
      if(b.steps && b.steps[0] && b.steps[0].text) allDoseTexts.push(b.steps[0].text);
    });
  });
  drugs.forEach(d => {
    (d.adult_dosing || []).forEach(b => {
      if(!b.steps || !b.steps[0] || !b.steps[0].text || !b.indication) return;
      const correctDose = b.steps[0].text;
      const distractors = pickDistractors(allDoseTexts, correctDose, 3);
      if(distractors.length < 3) return;
      pool.push({
        category: 'Dosing',
        prompt: `What is the correct adult dose of ${d.name} for ${b.indication}?`,
        answer: correctDose,
        options: shuffleArr([correctDose, ...distractors]),
      });
    });
  });

  // E — straightforward recall: certification level required.
  const allCertCombos = [];
  drugs.forEach(d => {
    if(d.certification && d.certification.length){
      const combo = d.certification.join(', ');
      if(allCertCombos.indexOf(combo) === -1) allCertCombos.push(combo);
    }
  });
  drugs.forEach(d => {
    if(!d.certification || !d.certification.length) return;
    const correctCert = d.certification.join(', ');
    const distractors = pickDistractors(allCertCombos, correctCert, 3);
    if(distractors.length < 3) return;
    pool.push({
      category: 'Certification',
      prompt: `What certification level(s) may administer ${d.name}?`,
      answer: correctCert,
      options: shuffleArr([correctCert, ...distractors]),
    });
  });

  // F — straightforward recall: onset of action.
  const allOnsets = [];
  drugs.forEach(d => { if(d.onset && allOnsets.indexOf(d.onset) === -1) allOnsets.push(d.onset); });
  drugs.forEach(d => {
    if(!d.onset) return;
    const distractors = pickDistractors(allOnsets, d.onset, 3);
    if(distractors.length < 3) return;
    pool.push({
      category: 'Onset',
      prompt: `What is the onset of action for ${d.name}?`,
      answer: d.onset,
      options: shuffleArr([d.onset, ...distractors]),
    });
  });
...
// Interleave the categories round-robin so the 150-question test is a mix
// rather than 33 classification questions in a row, then trim to exactly
// DRUG_QUIZ_TOTAL (or however many faithful questions exist, if fewer).
function generateDrugQuiz(){
  const pool = buildDrugQuizPool();
  const buckets = {};
  const order = [];
  shuffleArr(pool).forEach(q => {
    if(!buckets[q.category]){ buckets[q.category] = []; order.push(q.category); }
    buckets[q.category].push(q);
  });
  const chosen = [];
  let round = 0;
  let addedAny = true;
  while(chosen.length < DRUG_QUIZ_TOTAL && addedAny){
    addedAny = false;
    for(let i = 0; i < order.length; i++){
      ...
      if(bucket[round]){
        ...
        addedAny = true;
        if(chosen.length >= DRUG_QUIZ_TOTAL) break;
      }
    }
    round++;
  }
  return shuffleArr(chosen).slice(0, DRUG_QUIZ_TOTAL);
}

function renderDrugQuiz(){
  const tpl = document.getElementById('tpl-drug-quiz').content.cloneNode(true);
  app.innerHTML = '';
  app.appendChild(tpl);
  addBackBtn(goHome);

  const pre = document.getElementById('quizPre');
  ...
  if(quizState.ended){
    ...
    document.getElementById('startQuizBtn').addEventListener('click', () => {
      Object.assign(quizState, {
        ...
        questions: generateDrugQuiz(),
        index: 0, correctCount: 0, missed: [],
      });
      render();
    });
  }
...
function wireQuizLive(){
  document.getElementById('quizQuitBtn').addEventListener('click', () => {
    if(!confirm('End the test early? Your score so far will be shown.')) return;
...
function renderQuizQuestion(){
  ...
  if(!q){ quizState.active = false; quizState.ended = true; render(); return; }
  ...
  document.getElementById('quizScoreLabel').textContent =
    `Score: ${quizState.correctCount}/${quizState.index}`;
  document.getElementById('quizProgressFill').style.width =
    `${Math.round((quizState.index / quizState.questions.length) * 100)}%`;
  ...
  q.options.forEach(opt => {
    ...
    btn.textContent = opt;
    btn.addEventListener('click', () => selectQuizOption(opt, q));
...
function selectQuizOption(picked, q){
  const optionsWrap = document.getElementById('quizOptions');
  const correct = picked === q.answer;
  if(correct) quizState.correctCount++;
  else quizState.missed.push({prompt: q.prompt, correct: q.answer, picked});

  Array.from(optionsWrap.children).forEach(btn => {
    btn.classList.add('disabled');
    if(btn.textContent === q.answer) btn.classList.add('correct');
    else if(btn.textContent === picked) btn.classList.add('incorrect');
  });

  const feedback = document.getElementById('quizFeedback');
  feedback.classList.remove('hidden');
  feedback.textContent = correct ? 'Correct.' : `Correct answer: ${q.answer}`;

  document.getElementById('quizScoreLabel').textContent =
    `Score: ${quizState.correctCount}/${quizState.index + 1}`;

  const nextBtn = document.getElementById('quizNextBtn');
  nextBtn.classList.remove('hidden');
  nextBtn.querySelector('.cta-label').textContent =
    (quizState.index + 1 >= quizState.questions.length) ? 'See Results' : 'Next Question';
}

function renderQuizSummary(){
  const total = quizState.questions.length || 1;
  const answered = quizState.correctCount + quizState.missed.length;
  const pct = answered ? Math.round((quizState.correctCount / answered) * 100) : 0;
  const body = document.getElementById('quizSummaryBody');
  body.innerHTML = '';

  const scoreEl = document.createElement('p');
  scoreEl.className = 'quiz-summary-score';
  scoreEl.textContent = `${quizState.correctCount} / ${answered}`;
  body.appendChild(scoreEl);

  const pctEl = document.createElement('p');
  pctEl.className = 'quiz-summary-pct';
  pctEl.textContent = answered < total
    ? `${pct}% correct — ended early (${answered} of ${total} answered)`
    : `${pct}% correct`;
  body.appendChild(pctEl);

  if(quizState.missed.length){
    const title = document.createElement('p');
    title.className = 'quiz-missed-title';
    title.textContent = `Review — ${quizState.missed.length} missed`;
    body.appendChild(title);
    quizState.missed.forEach(m => {
      const item = document.createElement('div');
      item.className = 'quiz-missed-item';
      const strong = document.createElement('strong');
      strong.textContent = m.prompt;
      item.appendChild(strong);
      item.appendChild(document.createElement('br'));
      item.appendChild(document.createTextNode(`Correct: ${m.correct}`));
      item.appendChild(document.createElement('br'));
      item.appendChild(document.createTextNode(`You chose: ${m.picked}`));
  ...
  document.getElementById('quizHomeBtn').addEventListener('click', goHome);
}
