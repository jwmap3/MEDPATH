...
Static site, no backend, installable as an offline-capable PWA. Includes a "Run An Arrest" timer/documentation tool and a run-report export for every protocol. Covers a 71-protocol / 33-drug set — see **Status** below.
...
Every time `index.html`, `css/style.css`, or `js/app.js` changes, bump `CACHE_NAME` in `sw.js` (e.g. `medpath-v2` → `medpath-v3`) before uploading. The service worker caches files by name, not by content — without a version bump, phones that already have MEDPATH installed can end up running a stale mix of old and new files (e.g. an old page with a new script that expects a button the old page doesn't have), which throws errors on load. A pure `data/protocols.json` update doesn't need a bump — it'll just refresh a load or two behind, not break anything.
